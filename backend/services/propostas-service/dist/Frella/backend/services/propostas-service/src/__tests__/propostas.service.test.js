...
describe("PropostaRepository", () => {
    ...
    it("deve criar uma proposta e retornar os dados", async () => {
        ...
        const resultado = await PropostaRepository_1.PropostaRepository.create({
            titulo: "Reforma banheiro",
            descricao: "Desenvolvimento de sistema",
            valor: 3500,
            status: "CRIADA",
        ...
        expect(PropostaRepository_1.PropostaRepository.create).toHaveBeenCalledTimes(1);
    });
    it("deve buscar todas as propostas", async () => {
        PropostaRepository_1.PropostaRepository.findAll
            .mockResolvedValue([mockProposta]);
        const resultado = await PropostaRepository_1.PropostaRepository.findAll();
